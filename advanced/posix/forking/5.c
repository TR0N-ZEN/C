...
int main ()
{
  ...
  while (process_counter < process_maximum)
  {
    ...
    {
      int c = process_counter*4 + 5; // since this approach causes the child processes to exit with about 4 seconds time delta it seems that the enclosing codeblock is ran in turns between the parent process and a child process; 6.c,7.c,8.c,9.c together suggests that the child process may get a copy of each variable of the parent at time of creation of the child process
      sleep(c);
